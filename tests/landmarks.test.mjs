...
const blackGate=landmarks.records.find(r=>r.data.id==='black-gate');
assert.ok(blackGate, 'Black Gate landmark is present');
assert.equal(blackGate.data.name, 'Black Gate');
landmarks.dispose();assert.equal(landmarks.root.parent,null);
console.log(`PASS: ${landmarks.records.length} selectable landmarks, ${low} low-poly triangles / ${high} enhanced triangles, lazy effects, relief anchoring and resource reuse`);
